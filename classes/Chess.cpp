...
void Chess::FENtoBoard(const std::string &fen)
{
    ...
    for (char c : fen)
    {
        ...
        //* PIECE PLACEMENT *//
        if (c >= '0' && c <= '9')
        { // numerics indicate empty spaces
            int num_empty = c - '0';
            x += num_empty - 1;
            continue;
        }
...
bool Chess::canBitMoveFrom(Bit &bit, BitHolder &src)
{
    ...
    if (pieceIsWhite != currentIsWhite)
        return false;
...
void Chess::generatePawnMoves(std::vector<BitMove> &moves, BitboardElement pawns, const BitboardElement empty, const BitboardElement enemies, char color)
{
    ...
    BitboardElement singleMoves = (color == WHITE) ? (pawns.getData() << 8) & empty.getData() : (pawns.getData() >> 8) & empty.getData();
    BitboardElement doubleMoves = (color == WHITE) ? ((singleMoves.getData() & rank3) << 8) & empty.getData() : ((singleMoves.getData() & rank6) >> 8) & empty.getData();

    BitboardElement capturesLeft = (color == WHITE) ? ((pawns.getData() & notHFile) << 7) & enemies.getData() : ((pawns.getData() & notAFile) >> 9) & enemies.getData();
    BitboardElement capturesRight = (color == WHITE) ? ((pawns.getData() & notAFile) << 9) & enemies.getData() : ((pawns.getData() & notHFile) >> 7) & enemies.getData();
